...
sap.ui.define([
 ...
 ], function (BaseController, jQuery, JSONModel, MessageToast, Label, Popover, DateFormat, Fragment, Log, unifiedLibrary) {
   "use strict";

   var CalendarDayType = unifiedLibrary.CalendarDayType;

   return BaseController.extend("sap.ui.app01.controller.PRO.PRO02", {

    onInit: function(oEvent){
      var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
      oRouter.getRoute("pro02").attachPatternMatched(this._onObjectMatched, this);      
    },
        ...
        success: function(response){
          ...
          for (var i = 0; i < response.length; i++) {
            ...
            for (var x = 0; x < lengthAppointments; x++) {
              let dateAppointmentStart = response[i]['appointments'][x]['start'].date;
              response[i]['appointments'][x]['start'] = new Date(dateAppointmentStart);

              let dateAppointmentEnd = response[i]['appointments'][x]['end'].date;
              response[i]['appointments'][x]['end'] = new Date(dateAppointmentEnd);
            }
          }
