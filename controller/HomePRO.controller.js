...
sap.ui.define([
   ...
   "use strict";
   return BaseController.extend("sap.ui.app01.controller.HomePRO", {

       onInit: function(oEvent) {
           var thes = this;
           var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
           oRouter.getRoute("homePRO").attachPatternMatched(this._onObjectMatched, this);          
       },      
      ...
      getNodos: function(oEvent){
        ...
        statusKpiN1 = thes.setSemaforoKPIN1('L2'),
        statusKpiN2 = thes.setSemaforoKPIN1('L3'),
        statusKpiN3 = thes.setSemaforoKPIN2('L2'),
        statusKpiN4 = thes.setSemaforoKPIN2('L3');
        ...
        $.ajax({
          ...
          success:  function (response) {
            for (let index = 0; index < response.TileCollection.length; index++) {
              if(response.TileCollection[index].view == 'pro07'){
                response.TileCollection[index].color = statusKpiN1;
              } else if (response.TileCollection[index].view == 'pro08'){
                response.TileCollection[index].color = statusKpiN2;
              }  else if (response.TileCollection[index].view == 'pro09'){
                response.TileCollection[index].color = statusKpiN3;
              } else if (response.TileCollection[index].view == 'pro10'){
                response.TileCollection[index].color = statusKpiN4;
              }else {
...
thes.getRouter().navTo('home');
      ...
      setSemaforoKPIN1: function(linea_produccion){
        var thes = this,
        parametros = {
      ...
      setSemaforoKPIN2: function(linea_produccion){
        var thes = this,
        parametros = {
          '_LineaProduccion': linea_produccion
        },
          ...
          onClose: function(oAction) {
            if(oAction == "Actividades"){
              ...
              if(selectedPath.length > 0){
                ...
                if(tipo_evento == 'PRODUCCIÓN'){
                  view = 'okr3-1-LP';
                } else if(tipo_evento == 'EFICIENCIA'){
                  view = 'okr3-2-LP';
                } else if(tipo_evento == 'OEE'){
                  view = 'okr3-3-LP';
                }
      ...
      onFilerByMonth: function(){
        ...
        $.ajax({
          data: parametros,
          url:   'model/PA/ListarPaByMonth.php', 
          type:  'POST',
          async: false,
          beforeSend: function () {
          },
      ...
      onFilterByDates: function(){
        ...
        $.ajax({
          data: parametros,
          url:   'model/PA/ListarPaByDates.php', 
          type:  'POST',
          async: false,
          beforeSend: function () {
          },
