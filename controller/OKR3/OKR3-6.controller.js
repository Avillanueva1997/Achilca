...
sap.ui.define([
 ...
 './Formatter'
   ...
   return BaseController.extend("sap.ui.app01.controller.OKR3.OKR3-6", {

    onInit: function(oEvent){
      var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
      oRouter.getRoute("okr3-6").attachPatternMatched(this._onObjectMatched, this);      
    },
    ...
    getScada: function(){
        ...
        oMonth = thes.byId("btnFilterMonth").getSelectedKey(),
        oMonthCode = thes.getCodeMonth(oMonth),
        dateToday,
        ...
        var date;
        if(oYear == year && month == oMonthCode){
            date = new Date();
            type = false;
        } else {
            date = new Date(oYear, oMonthCode + 1, 0);
            type = true;
        }
        dateToday = oYear + '-' + ("0" + (oMonthCode +1)).slice(-2) + '-' + ("0" + date.getDate()).slice(-2);
    ...
    onReporteProducción: function(oEvent){
        ...
        oKeyLineaProduccion = thes.byId('idIconTabBarNoIcons').getSelectedKey(),
        turno = 'A',
        lastRecord = thes.getLastRecordByTurnoMonth(oKeyYear, codeMonth, oKeyLineaProduccion, turno),
        date = new Date();

        if(lastRecord.rango || (!lastRecord.rango && !lastRecord.evaluation)){
    ...
    evaluateButtons: function(){
        ...
        if(area == 'CALIDAD'){
            thes.byId("btnCierre").setVisible(false);
            thes.byId("btnReporte").setVisible(false);            
        } else {
            thes.byId("btnCierre").setVisible(true);
            thes.byId("btnReporte").setVisible(true);   
        }
    },
    sendReporteProducción: function(){
        var thes = this;
        sap.m.MessageBox.confirm("¿Qué es lo que desea enviar?", {
            actions: ["RP - Día", "RP - Noche", MessageBox.Action.CLOSE],
            emphasizedAction: MessageBox.Action.CLOSE ,
            onClose: function(oAction) {
              if(oAction == "RP - Día"){
                thes.sendPDF("A");
              } else if(oAction == "RP - Noche"){
                thes.sendPDF("B");
              }
            }
          });
    ...
    sendPDF: function(turno){
        ...
        if(turno == 'A'){
            var fechaIni = date.setHours(8,0,0);
            fechaIni = new Date(fechaIni);
            var fechaFin = date.setHours(20, 15,0);
            fechaFin = new Date(fechaFin);
            var ev = false;

            if(newFecha >= fechaIni && newFecha <= fechaFin) {
                date = thes.dateToMysqlDate(date);
                ev = true;
            } else {
                ev = false;
            ...
            var fechaIni = date.setDate(date.getDate() - 1);
            fechaIni = new Date(fechaIni);
            fechaIni = fechaIni.setHours(20,0,0);
            fechaIni = new Date(fechaIni);
            var datee = new Date();
            var fechaFin = datee.setHours(8,15,0);
            fechaFin = new Date(fechaFin);
            var ev = false;

            if(newFecha >= fechaIni && newFecha <= fechaFin) {
                date = thes.dateToMysqlDate(fechaIni);
                ev = true;
            } else {
                ev = false;
        ...
        if(ev){
            ...
            var stringDownload =  'Reporte de Producción ACH - ' + date + '.pdf';
                sap.ui.core.BusyIndicator.show();

                var docDefinition = thes.generatePDF(date, turno);
                ...
                MessageToast.show("No puede enviar el reporte!");
            }
    ...
    generatePDF: function(date, turno){
        debugger;
        var thes = this,
        ...
        dataTable = thes.returnReporteDetalle('L2', date, turno),
        dataTableSecond = thes.returnReporteDetalle('L3', date, turno),
        cabeceraTable = [{text: 'Inicio', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Fin', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Sistema', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Evento', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Comentario', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text:'Horas', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}],
        cabeceraTableSecond = [{text: 'Inicio', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Fin', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Sistema', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Evento', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text: 'Comentario', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}, {text:'Horas', fillColor: '#215967', style: 'styleHeader', alignment: 'center'}],
        contentTable = [],
        contentTableSecond = [],
        dataCabecera = thes.getOkrByCode(date, 'L2', turno),
        dataCabeceraSecond = thes.getOkrByCode(date, 'L3', turno),
        ...
        fecha = (typeof dataCabecera === 'object' && dataCabecera !== null) ? dataCabecera.fecha: date,
        ...
        downtime = 0,
        bar_chargSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.bar_charg: 0,
        bar_chargSecond = bar_chargSecond + ' TM',
        bar_procesSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.bar_proces: 0,
        bar_procesSecond = bar_procesSecond + ' TM',
        bar_rejectSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.bar_reject: 0,
        bar_rejectSecond = bar_rejectSecond + ' TM',
        produccion_netaSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.produccion_neta: 0,
        produccion_neta = produccion_netaSecond + ' TM',
        produccion_acumuladoSecond = (typeof dataCabeceraSecond === 'object' && dataCabecera !== null) ? dataCabeceraSecond.produccion_acumulado: 0,
        produccion_acumuladoSecond = produccion_acumuladoSecond + ' TM',
        fechaSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.fecha: date,
        turno_cabeceraSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.turno: turno,
        coordinadorSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.coord: '',
        diametroSecond = (typeof dataCabeceraSecond === 'object' && dataCabeceraSecond !== null) ? dataCabeceraSecond.diametro: '',
        downtimeSecond = 0,
        ...
        for (let index = 0; index < dataTableSecond.length; index++) {
            let inicio = dataTableSecond[index].inicio;
            inicio = moment(inicio, "HH:mm");
            let fin = dataTableSecond[index].fin;
            fin = moment(fin, "HH:mm");
            var duration = moment.duration(fin.diff(inicio));
            var hours = parseInt(duration.asHours());
            var minutes = parseInt(duration.asMinutes())-hours*60;
            var final = ((hours * 60) + minutes) / 60;
            dataTableSecond[index].horas = final.toFixed(2);
            downtimeSecond += Number(dataTableSecond[index].horas);
        }

        downtimeSecond = downtimeSecond.toFixed(1);
        downtimeSecond = downtimeSecond + ' h';
        
        if(fechaSecond != ''){
          let split = fechaSecond.split('-');
          fechaSecond = split[2] + '/' + split[1] + '/' + split[0];
        }
        ...
        contentTable.unshift(cabeceraTableSecond);

        for (let index = 0; index < dataTableSecond.length; index++) {
            let tempArray = [
                              {border: [false, false, false, false], text: dataTableSecond[index].inicio, style: 'styleBody'}, 
                              {border: [false, false, false, false], text: dataTableSecond[index].fin, style: 'styleBody'}, 
                              {border: [false, false, false, false], text: dataTableSecond[index].maquina, style: 'styleBody'}, 
                              {border: [false, false, false, false], text: dataTableSecond[index].causa, style: 'styleBody'}, 
                              {border: [false, false, false, false], text: dataTableSecond[index].comentario, style: 'styleBody'}, 
                              {border: [false, false, false, false], text: dataTableSecond[index].horas, style: 'styleBody'}
                            ];
            contentTableSecond.push(tempArray);          
          }
  
          contentTableSecond.unshift(cabeceraTable);
          ...
          content: [
            {
              ...
              table: {
                ...
                headerRows: 2,
                body: [
                  [{image: logoMepsaAch, fit:[30,100], colSpan: 2, border: [true, true, false, true]}, {}, { text: 'REPORTE DE PRODUCCIÓN ' + 'L2', style: 'tableHeader', colSpan: 3, border: [false, true, true, true] }, {}, {}],
            ...
            },
            {
                ...
                table: {
                  ...
                  headerRows: 2,
                  body: [
                    [{image: logoMepsaAch, fit:[30,100], colSpan: 2, border: [true, true, false, true]}, {}, { text: 'REPORTE DE PRODUCCIÓN ' + 'L3', style: 'tableHeader', colSpan: 3, border: [false, true, true, true] }, {}, {}],
                    [{border: [true, false, false, false], text: 'Barras cargadas:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: bar_chargSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, false, false], text:'Fecha:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: fechaSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, true, false], text:'', fillColor: '#eeece1'}],
                    [{border: [true, false, false, false], text: 'Barras Procesadas:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: bar_procesSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, false, false], text:'Turno:', fillColor: '#eeece1', style: "styleLabel"},{border: [false, false, false, false], text: turno_cabeceraSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, true, false], text:'', fillColor: '#eeece1'}],
                    [{border: [true, false, false, false], text: 'Barras Rechazadas:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: bar_rejectSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, false, false], text:'Coordinador:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: coordinadorSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, true, false], text:'', fillColor: '#eeece1'}],
                    [{border: [true, false, false, false], text: '', fillColor: '#eeece1'}, {border: [false, false, false, false], text: '', fillColor: '#eeece1'}, {border: [false, false, false, false], text: '', fillColor: '#eeece1'}, {border: [false, false, false, false], text: '', fillColor: '#eeece1'}, {border: [false, false, true, false], text: '', fillColor: '#eeece1'}],
                    [{border: [true, false, false, false], text: 'Producción:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: produccion_netaSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, false, false], text: 'Medida:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, false], text: diametroSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, true, false], text: 'Downtime', fillColor: '#eeece1', style: 'styleLabel'}],
                    [{border: [true, false, false, true], text: 'Producción Acumulado:', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, true], text: produccion_acumuladoSecond, fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, false, true], text: '', fillColor: '#eeece1', style: "styleLabel"}, {border: [false, false, false, true], text: '', fillColor: '#eeece1', style: 'styleValue'}, {border: [false, false, true, true], text: downtimeSecond, fillColor: '#eeece1', style: 'styleLabel'}]
                  ]
                }
              },
              { text: 'Detalle de Paradas', style: 'tableHeader', alignment: 'center'},
