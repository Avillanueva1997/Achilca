...
sap.ui.define([
 ...
 "sap/m/MessageToast"
   ...
   "use strict";
   return BaseController.extend("sap.ui.app01.controller.SPRO.SPRO15", {

    onInit: function(oEvent){
      var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
      oRouter.getRoute("spro15").attachPatternMatched(this._onObjectMatched, this);      
    },
    _onObjectMatched: function(oEvent) {
      var thes = this;
      thes.cargarFactorLinea();
      thes.getLineaProduccion();
      thes.getDiametro();
    },
    cargarFactorLinea: function(){
      var thes = this;
      $.ajax({
        url:   'model/SPRO/ListarBPM.php', 
        type:  'GET',
        dataType: 'json',
        async: false,
        beforeSend: function () {
        ...
        success:  function (response) {
          ...
          thes.byId("TableSPRO15").setModel(oModel);
        },
    ...
    onAdd: function(oEvent){
      ...
      $.ajax({
        ...
        success:  function (response) { 
          var data = thes.byId("TableSPRO15").getModel().getData();
          ...
          data.push(response);
          thes.byId("TableSPRO15").getModel().refresh();
        },
        error: function (xhr, ajaxOptions, thrownError) {
          alert(xhr.status);
          alert(thrownError);
    ...
    onDelete:function(oEvent){
      var thes = this,
      index,
      datosTabla = thes.byId("TableSPRO15").getModel().getData(),
      selectData = thes.byId("TableSPRO15").getSelectedContextPaths();
      ...
      thes.byId("TableSPRO15").getModel().setData(newTable);
      thes.byId('TableSPRO15').removeSelections();
      thes.byId("TableSPRO15").getModel().refresh();
    },
    onSave: function(oEvent){
      var thes = this;       

      var datosTabla = thes.byId("TableSPRO15").getModel().getData();
    ...
    getDiametro: function(){
      ...
      $.ajax({
        ...
        success: function(response){
          response = JSON.parse(response); 
          var oModel = new sap.ui.model.json.JSONModel(response);  
          thes.getView().setModel(oModel,"cbxBall");  
        },
        error: function(xhr, ajaxOptions, thrownError){
          alert(xhr.status);
          alert(thrownError);
