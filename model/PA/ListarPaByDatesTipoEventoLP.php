...
$sql = "select code, tipo_evento, linea_produccion, evento, fecha_evento 
from {$_Table}_cab
where (fecha_evento between '".$_From."' and '".$_To."') and tipo_evento = '".$_TipoEvento."' and linea_produccion = '".$_LineaProduccion."'
...
for ($i=0; $i < sizeof($Eventos); $i++) {
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        $Eventos[$i]['Actividades'][] = array(
            ...
            'linea_produccion' => $Eventos[$i]['linea_produccion']
        );
    } 	  
